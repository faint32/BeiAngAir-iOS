...
+ (CGRect)rightBtnFrame
{
    if (IsiOS7Later)
        ...
        return Rect(258.0f, 2.0f, [[self class] barBtnSize].width, [[self class] barBtnSize].height);
}
...
+ (CGRect)titleViewFrame
{
    if (IsiOS7Later)
        ...
        return Rect(65.0f, 2.0f, 190.0f, 40.0f);
}
...
- (void)setTitle:(NSString *)strTitle color:(UIColor *)color
{
    [_labelTitle setText:strTitle];
...
- (void)setLeftBtn:(UIButton *)btn
{
    ...
    if (_btnLeft)
    {
        if (IsiOS7Later)
            ...
            _btnLeft.frame = Rect(2.0f, 2.0f, [[self class] barBtnSize].width, [[self class] barBtnSize].height);
